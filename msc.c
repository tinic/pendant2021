...
static uint8_t diskSectorBuffer[USBD_SECTOR_SIZE];

#define MSC_BUFFER_SIZE 128
static uint8_t massStorageBuffer[MSC_BUFFER_SIZE];

static int32_t g_TotalSectors = 0;

/* USB flow control variables */
static uint8_t g_u8BulkState = BULK_NORMAL;
static uint8_t g_u8Prevent = 0;
static uint8_t volatile g_u8Remove = 0;
static uint8_t volatile g_u8MscOutPacket = 0;
static uint8_t g_au8SenseKey[4] = {0};
...
/* CBW/CSW variables */
static struct CBW g_sCBW = {0};
static struct CSW g_sCSW = {0};
...
void MSC_Init(void)
{
    ...
    g_TotalSectors = emfat.disk_sectors;
#endif  // #ifdef BOOTLOADER
}
...
{
    memset(massStorageBuffer, 0, sizeof(massStorageBuffer));
    ...
    if (g_u8MscOutPacket)
    {
        ...
        if (g_u8BulkState == BULK_CBW)
        {
            ...
            case UFI_READ_10:
            {
                ...
                while(1)
                {
                    if (total > USBD_SECTOR_SIZE)
                    {
                        len = USBD_SECTOR_SIZE;
                        MSC_ReadMedia(g_u32LbaAddress, USBD_SECTOR_SIZE, (uint8_t *)g_u32StorageBase);
                        MSC_BulkIn(g_u32StorageBase, USBD_SECTOR_SIZE);
                        g_u32LbaAddress += len;
                        total -= len;
                    }
                    else
            ...
            case UFI_WRITE_10:
            {
                ...
                if (g_sCBW.bmCBWFlags == 0x00)      /* OUT */
                {
                    ...
                    while(1)
                    {
                        ...
                        {
                            len = USBD_SECTOR_SIZE;
                            MSC_BulkOut(g_u32StorageBase, USBD_SECTOR_SIZE);
                            MSC_WriteMedia(g_u32LbaAddress, USBD_SECTOR_SIZE, (uint8_t *)g_u32StorageBase);
                            g_u32LbaAddress += len;
                            total -= len;
                        }
                        else
...
{
#ifdef BOOTLOADER
    emfat_read(&emfat, buffer, addr / 512, size / 512);
#endif  // #ifdef BOOTLOADER
}
...
{
#ifdef BOOTLOADER
    emfat_write(&emfat, buffer, addr / 512, size / 512);
#endif  // #ifdef BOOTLOADER
}
